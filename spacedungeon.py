...
def attack_player(player, opponent):
    rand_damage = random.randint(8, 32)
    hit_damage = rand_damage * player['attack'] // opponent['defense']
    opponent['HP'] -= hit_damage
    print(rand_damage, " damage!")
    return opponent

def attack_monster(opponent, player):
    rand_damage = random.randint(8, 32)
    hit_damage = rand_damage * opponent['attack'] // player['defense']
